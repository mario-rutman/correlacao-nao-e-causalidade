...
#### Tenho visto em _lives_, em cursos de Estatística e de Análise de Dados a afirmação de que 'correlação não é causalidade', indicando que algo falta à correlação, como se estivesse incompleta sem a causalidade. Não penso assim. O fato de uma análise de correlação não ter revelado ou encontrado causas não a torna deficiente.

#### Minha argumentação será no sentido de mostrar que um estudo de correlação não tem necessariamente que  identificar ou referir-se à causalidade. Para defender meu entendimento abordarei 3 pontos: buscar causas é tarefa totalmente diferente de analisar correlações, são instâncias heterogêneas; sob certa perspectiva é possível buscar correlações sem dar qualquer importância à causalidade; e, a noção de causa e efeito, embora irresistível, não passa de um hábito sem fundamento.

#### Para tratar do tema apresentarei resumidamente alguns conceitos filosóficos uteis.
...
#### É impensável para Platão que o conhecimento tenha alguma relação com a análise de um conjunto de observaçãoes ou experimento. A valorização de Matemática em sua filosofia é em razão de sue caráter abstrato, da distância que esta disciplina mantém em relação aos sentidos. 
...
#### Segundo __Descartes__ conhecer é conduzir a razão à moda da Matemática, isto é rejeitar o provável e as experiências enganosas, e considerar somente aquilo que é impossível duvidar.
...
#### Num estudo de correlação coletam-se as observações, identificam-se as variáveis e estuda-se __quanto__ estas se correlacionam. A forma de se obter o grau de uma correlação é bastante engenhosa, mas é medição e contagem, não é atividade especulativa ou teórica. Neste cenário, não há como surgir afirmação __fundamentada__de que __$A$__ causa __$B$__.
...
#### O filósofo escocês considera a causalidade um ilusão, uma força do hábito que adquirimos ao ver repetidamente grupo coisas semelhantes seguidas por outro grupo de coisas semelhantes, que nada mais é que correlação. A relação de causa e efeito não está na natureza, nas coisas nem na razão, é apenas repetida conjugação de fenômenos.
...
#### Se você é um cientista de dados e sua investigação de correlação foi questionada por não ser 'causalidade', pode responder que admira o trabalho especulativo-científico, mas que seu trabalho é encontrar padrões sem fazer inferências sobre seu estatuto metafísico.

#### Pode refutá-lo afirmando que não tem qualquer interesse no assunto 'causalidade', que é 'um homem no mundo', que não pode esperar o conhecimento das causas para decidir, que troca sem pesar o conhecimento das profundezas do oceano pela certeza que seu navio não será destruído pelos arrecifes.

#### Sugeriria, para não ferir suscetibilidades, com muito tato, objetá-lo que 'causalidade' é apenas um costume, um vício, e como tal deve ser evitado.

#### Por fim, ofereça um café a quem veio indagá-lo sobre faltar 'causalidade' em sua correlação e diga que sabia que ele viria tratar do tema, pois sua pesquisa indicou que há forte correlação entre as variáveis __$MUC$__^['mostrar uma correlação'] e __$AADCNC$__^['aparecer alguém dizendo que correlação não é causalidade'].
